...
public class DBConnect {
    ...
    public ResultSet validatePwd(String sql33) {
        String message = openDB();
        if (message.equals("Opened")) {
            try {
                return stm.executeQuery(sql33);
            } catch (Exception e) {
                return null;
            }
        } else {
        ...
        if (message.equals("Opened")) {
            try {
                ...
                if (!rst.isBeforeFirst()) {
                    ...
                    html += "<div class='container'><br><br><h4>    Nothing matched your search!</h4><br><br><br><br></div>";
                } else {
                    html = createHTMLTable(sql);
